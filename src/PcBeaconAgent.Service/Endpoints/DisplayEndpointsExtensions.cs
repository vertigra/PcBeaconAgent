    ...
    public static class DisplayEndpointsExtensions
    {
        ...
        public static IEndpointRouteBuilder MapDisplayServiceEndpoints(this IEndpointRouteBuilder app, AppSettings settings)
        {
            ...
            displayGroup.MapGet("/list", ([FromServices] DisplayController controller) =>
            {
                try
                {
                    return Results.Json(controller.GetDisplays(), BeaconJsonContext.Default.ListDisplayDeviceDtos, statusCode: StatusCodes.Status200OK);
                }
                catch(Exception ex)
                {
            ...
            displayGroup.MapPost("/disable", async ([FromBody] DisableRequest request, [FromServices] DisplayController controller) =>
            {
                try
                {
                    ...
                    return Results.Json(new MessageResponse("Display disabled"), BeaconJsonContext.Default.MessageResponse, statusCode: StatusCodes.Status200OK);
                }
                catch (Exception ex)
                {
                    return Results.BadRequest(new MessageResponse(ex.Message));
            ...
            displayGroup.MapPost("/restore", async ([FromServices] DisplayController controller) =>
            {
                try
                {
                    ...
                    return Results.Json(new MessageResponse("Displays restored"), BeaconJsonContext.Default.MessageResponse, statusCode: StatusCodes.Status200OK);
                }
                catch(Exception ex)
                {
                    return Results.BadRequest(new MessageResponse(ex.Message));
