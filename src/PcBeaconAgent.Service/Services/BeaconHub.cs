...
{
    public class BeaconHub(AppSettings mSettings, ILogger<BeaconHub> mLogger) : Hub
    {
        ...
        {
            if (string.IsNullOrEmpty(mSettings.Server.ApiKey)) return true;
            ...
            return string.Equals(provided, mSettings.Server.ApiKey, StringComparison.Ordinal);
        }
        ...
        private BeaconDevice GetLocalDeviceData()
        {
            ...
            return new BeaconDevice
            {
                ...
                ApiPort = mSettings.Server.ApiPort,
        ...
        private static readonly Action<ILogger, string, Exception?> LogUnauthorizedAction =
            LoggerMessage.Define<string>(LogLevel.Warning, new EventId(1, "Unauthorized"), "Rejected hub connection {ConnectionId}: invalid or missing API key");

        private static readonly Action<ILogger, Exception?> LogConnectedAction =
            LoggerMessage.Define(LogLevel.Information, new EventId(2, "Connected"), "Client connected");

        private static readonly Action<ILogger, string?, Exception?> LogDisconnectedAction =
            LoggerMessage.Define<string?>(LogLevel.Information, new EventId(3, "Disconnected"), "Client disconnected by reason {reason}");
