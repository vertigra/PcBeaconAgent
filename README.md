...
### Параметры запуска (CLI Флаги)
По умолчанию агент дублирует все логи в окно консоли. Для «тихого» запуска (например, при вызове из сторонних скриптов или планировщика) предусмотрены флаги отключения консольного вывода:

```bash
# Запуск в тихом режиме (логи пишутся только в файл)
